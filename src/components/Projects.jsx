...
function Projects() {
  ...
  
 {
  ...
  return (
    <section id="project" className="py-5 projects-section">
      <Container>
        <h2 className="display-5 fw-bold text-center mb-5">Featured <span className="text-gold">Projects</span></h2>

        {/* አማርኛ */}
  <p className="mt-3 mx-auto" style={{ maxWidth: '850px', fontSize: '1.05rem' }} data-aos="fade-up">
  ...
  {/* እንግሊዝኛ */}
  <p className="mx-auto" style={{ maxWidth: '850px', fontStyle: 'italic', fontSize: '0.95rem' }} data-aos="fade-up" data-aos-delay="200">
    The projects showcased here represent my ability to transform ideas into functional digital solutions. Each work reflects my commitment to delivering quality and solving real-world problems through technology.
          ...
          {myProjects.map((project, index) => (
            <Col key={index} xs={12} sm={6} lg={4} className="mb-4">
              <Card className="h-100 border-0 shadow-sm project-card overflow-hidden">
                ...
                <Card.Body>
                  <Badge className="project-badge mb-2">{project.category}</Badge>
                  <Card.Title className="fw-bold">{project.title}</Card.Title>


                  {/* የአማርኛው ጽሁፍ */}
                      <Card.Text className="fw-normal mb-1" style={{ fontSize: '0.9rem' }}>
                        {project.descriptionAm}
                      </Card.Text>

                      {/* የእንግሊዝኛው ጽሁፍ (ካንተ ኮድ ጋር የተቀናጀ) */}
                      <Card.Text className="small" style={{ minHeight: '50px', fontStyle: 'italic' }}>
                        {project.descriptionEn}
                      </Card.Text>
                 
                  <div className="mb-3">
                    {project.skills.map((s, i) => (
                      <Badge key={i} pill className="project-skill me-1" style={{ fontSize: '0.7rem' }}>{s}</Badge>
                    ))}
                  </div>

                  {/* ጨዋታ ከሆነ "Play" ካልሆነ "View" በተን ያሳያል */}
                  {project.isGame ? (
                    <Button variant="primary" className="w-100 fw-bold" onClick={() => setShowGame(true)}>
                      Play Now! 🎮
                    </Button>
                  ) : (
        ...
        <Modal show={showGame} onHide={() => setShowGame(false)} size="lg" centered>
          <Modal.Header closeButton>
            <Modal.Title className="fw-bold text-gold">Snake Game - Have Fun!</Modal.Title>
          </Modal.Header>
